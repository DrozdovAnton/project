...
# Глобальные переменные для хранения графа и данных
G = None
labels = None
tasks = []  # Хранение задач как объектов Task
employee_id_counter = 1  # Счетчик для назначения сотрудников
...
def create_task():
    global tasks, employee_id_counter
    if request.method == 'POST':
        ...
        if st1_id is not None and st2_id is not None:
            ...
            task = Task(start_station, end_station, task_start_time, G, labels)
            task.employee_id = employee_id_counter
...
def load_test_data():
    global tasks, employee_id_counter
    for t in tasks_list:
        start_station, end_station, hour, minute = t
        st1_id = find_station_id(labels, start_station)
        st2_id = find_station_id(labels, end_station)

        if st1_id is not None and st2_id is not None:
            task_start_time = time(hour, minute)
            task = Task(start_station, end_station, task_start_time, G, labels)
            task.employee_id = employee_id_counter
            employee_id_counter += 1
            tasks.append(task)
...
def view_schedule():
    return render_template('view_schedule.html', tasks=tasks)
...
def edit_task(task_id):
    ...
    if request.method == 'POST':
        ...
        task['start_station'] = request.form['start_station']
        task['end_station'] = request.form['end_station']
        start_hour = int(request.form['start_hour'])
        start_minute = int(request.form['start_minute'])
    ...
    return render_template('edit_task.html', task=task, stations=stations)
